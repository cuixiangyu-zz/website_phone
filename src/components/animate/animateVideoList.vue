  ...
  <div id="main">
    ...
    <div class="main_fixed_top">
      <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect"background-color="#545c64"
        ...
        <el-menu-item index="1">animate</el-menu-item>
    ...
    <div class="job_content" id="animateList" ref="wrapper" v-if="tab===1">
      <ul
      ...
      >
        <li v-for="(item,index) in tableData.animateList.list" :key="index">
          <router-link
    ...
    <div class="job_content" id="animateFavorite" ref="wrapper" v-if="tab===2">
      <ul
          class="page-infinite-list job_lists"
          v-infinite-scroll="loadMore"
          infinite-scroll-disabled="loading"
          infinite-scroll-distance="50"
      >
        <li v-for="(item,index) in tableData.animateFavorite.list" :key="index">
          <router-link
              @click="changeDetialId(index)"
    ...
    <div class="job_content" id="animateHistory" ref="wrapper" v-if="tab===3">
      <ul
          class="page-infinite-list job_lists"
          v-infinite-scroll="loadMore"
          infinite-scroll-disabled="loading"
          infinite-scroll-distance="50"
      >
        <li v-for="(item,index) in tableData.animateHistory.list" :key="index">
          <router-link
              @click="changeDetialId(index)"
              :to="{ name: 'detail', params: { id: item.video.id }}"
          >
...
export default {
  ...
  methods: {
    ...
    loadMore() {
      ...
      if(this.tab===1){
        this.loading = true;
        this.noMore = false;
        getPageList(this.listQuery.animateList).then(res => {
          this.listQuery.animateList.currentPage = this.listQuery.animateList.pageNum;
          this.listQuery.animateList.pageNum = this.randomNum(1, res.PageInfo.pages);
          if (this.tableData.animateList.length <= 0) {
            this.tableData.animateList = res.PageInfo
          } else {
            this.tableData.animateList.list = this.tableData.animateList.list.concat(res.PageInfo.list)
          }
          this.loading = false;
        })
      }else if(this.tab===2&&this.listQuery.animateHistory.load){
        this.loading = true;
        this.noMore = false;
        getList(this.listQuery.animateFavorite).then(res => {
          if(res.pageNum>=res.pages){
            this.listQuery.animateFavorite.load = false;
          }else{
            this.listQuery.animateFavorite.pageNum = res.pageNum+1;
          }
          if (this.tableData.animateFavorite.length <= 0) {
            this.tableData.animateFavorite = res
          } else {
            this.tableData.animateFavorite.list = this.tableData.animateFavorite.list.concat(res.list)
          }
          this.loading = false;
        })
      }else if(this.tab===3&&this.listQuery.animateHistory.load){
        this.loading = true;
        this.noMore = false;
        getHistory(this.listQuery.animateHistory).then(res => {
          if(res.pageNum>=res.pages){
            this.listQuery.animateHistory.load = false;
          }else{
            this.listQuery.animateHistory.pageNum = res.pageNum+1;
          }
          if (this.tableData.animateHistory.length <= 0) {
            this.tableData.animateHistory = res
          } else {
            this.tableData.animateHistory.list = this.tableData.animateHistory.list.concat(res.list)
          }
          this.loading = false;
        })
      }else {
    ...
    getPageList() {
      this.noMore = false;
      var listQuery = sessionStorage.getItem('listQuery_animate_video')
      var refresh = sessionStorage.getItem('refresh_animate_video')
      if (listQuery !== null && refresh !== null && refresh === 'true') {
        this.listQuery = JSON.parse(listQuery)
      }
      this.listQuery.animateList.pageNum = this.listQuery.animateList.currentPage;
      getPageList(this.listQuery.animateList).then(res => {
        this.listQuery.animateList.pageNum = this.randomNum(1, res.PageInfo.pages);
        this.tab = 1;
        this.tableData.animateList = res.PageInfo
    ...
    getFavoriteList() {
      ...
      getList(this.listQuery.animateFavorite).then(res => {
        if (res.pageNum >= res.pages) {
          this.listQuery.animateFavorite.load = false;
          this.listQuery.animateFavorite.pageNum = res.pageNum
        } else {
          this.listQuery.animateFavorite.pageNum = res.pageNum + 1;
        }
        this.tab = 2;
    ...
    getHistoryList() {
      ...
      getHistory(this.listQuery.animateHistory).then(res => {
        if (res.pageNum >= res.pages) {
          this.listQuery.animateHistory.load = false;
          this.listQuery.animateHistory.pageNum = res.pageNum
        } else {
          this.listQuery.animateHistory.pageNum = res.pageNum + 1;
        }
        this.tab = 3;
    ...
    handleCurrentChange(index) {
      ...
      sessionStorage.setItem('refresh_animate_video', false)
      this.getPageList()
    },
    ...
    jump(videoid) {
      this.listQuery.animateList.pageNum = this.listQuery.animateList.currentPage;
      sessionStorage.setItem('listQuery_animate_video', JSON.stringify(this.listQuery))
      sessionStorage.setItem('refresh_animate_video', true)
  ...
  beforeRouteLeave(to, form, next) {
    this.listQuery.animateFavorite.pageNum = 1;
    this.listQuery.animateHistory.pageNum = 1;
    this.listQuery.animateList.pageNum = this.listQuery.animateList.currentPage;
    sessionStorage.setItem(
        "listQuery_animate_video",
